...
import styles from './bumenCheckSelect.less';
...
const TreeOrgCheckSelect = React.createClass({
    ...
    init() {
        ...
        let self = this;
        if(!!type && type == 'group'){
            serviceRequest(`${BASE_URL}/crm/hq/courseGroup/list`, {},
                function(ret) {
                    let newList = !!ret.data ? [...ret.data] : [];
                    newList.forEach((item) => {
                        item.key = item.id;
                        item.label = item.name;
                    })
                    self.setState({
                        all_org_list : newList,
                        all_org_num : ret.orgNum || newList.length
                    });
                },
            );
        }else{
            serviceRequest(`${BASE_URL}/crm/hq/dept/deptTreeQuery`, {},
                function(ret) {
                    self.setState({
    ...
    render() {
        ...
        const { type } = this.props;
        let title = !!type && type == 'group' ? '分组' : '部门';
        ...
        let leftLoopOrgSelectTreeNode = (data, show) => data.map(function(item) {
            ...
            if (item.children && item.children.length > 0) {
                ...
                return (
                    <TreeNode key = { item.key + '' } title = { item.type == 'dept' ? <span><Icon type = 'xiaoquguanli' className={styles.icon}/>{ item.label }</span> : <span>{ item.label  + (!!filter_key ? '' : `（${item.orgNum || 0}）`) }</span> } mark = { item.type } className={sub_node_show ? styles.block_tree_node : styles.hidden_tree_node} >
                      { leftLoopOrgSelectTreeNode(item.children) }
                    </TreeNode>
                  );
            } else {
                return <TreeNode key = { item.key + '' } title={ item.type == 'dept' ? <span><Icon type = 'xiaoquguanli' className={styles.icon}/>{ item.label }</span> : <span>{ item.label  + (!!filter_key ? '' : `（${item.orgNum || 0}）`) }</span> } mark = { item.type } className={(show || node_show) ? styles.block_tree_node : styles.hidden_tree_node} />;
            }
        });

        let rightLoopOrgSelectTreeNode = (data) => data.map(function(item) {
            return <TreeNode key = { item.key + '' } title={ item.type == 'dept' ? <span><Icon type = 'xiaoquguanli' className={styles.icon}/>{ item.label }</span> : item.label } mark = { item.type } className={ item.type == 'dept' && org_select.indexOf(item.key) > -1 ? styles.block_tree_node : styles.hidden_tree_node } />;
        });
                    ...
                    <div className={styles.org_to_select_tree}>
                        <div className={styles.tree_content}>
                            <Tree
                              ...
                              >
                                <TreeNode title = {`所有${title}（${all_org_num}）`} key="all_org">
                                    { org_list_tree_nodes }
                                </TreeNode>
                            </Tree>
                        </div>
                        <div className={styles.tree_content}>
                            <Tree
                                ...
                                onSelect={this.onOrgTreeSelect} selectedKeys={this.state.tree_select}
                              >
                                <TreeNode title = {`已选${title}（${!!org_select && org_select.length > 0 ? org_select.length : 0}）`} key="all_org">
