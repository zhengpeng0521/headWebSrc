			...
			<div className = 'manager_list_wrap' id = 'manager_list_wrap'>
				...
				{ !!incomeBars &&
					<div className = {styles.manager_income_number}>
						<p>收入 : <span className = { styles.incomeNum }>{ incomeBars.totalIncomeNum}</span>元</p>
	                	<p>数量 : <span>{ incomeBars.incomeCount}</span>笔</p>
					</div>
				}
				{ !!search &&
			...
			{ !!pagination &&
				<div className = 'manager_list_pagination_box' style = {{ bottom : !!table && table.isInDetail ? '49px' : '', width : !!table && ( table.isInDetail || table.isWidth ) ? '100%' : '' }} >
					<div className = 'manager_list_pagination'>
						<Pagination { ...pagination } current = { parseInt(pagination.pageIndex) + 1 } pageSizeOptions = {['20', '50', '100', '200']} />
					</div>
				</div>
			}
		</div>
