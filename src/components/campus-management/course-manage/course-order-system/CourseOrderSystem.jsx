...
function CourseOrderSystem({
    ...
    for(let i in courseOrderSystemModalData){
        ...
        tr_item.splice(tr_item.length - 1,1,
            <div className = { styles.tr_item } key = { 'tr_item_' + i + '_operation' } style = {{ width : `${thead[thead.length-1].width}` }}>
                ...
                <Popconfirm placement="top" title={<div>确定删除<span style={{ color: 'red' }}>{courseOrderSystemModalData[i].name}</span>？<div className={styles.bottom_text}>(注：若该课系有关联课程，则不可删除)</div></div>} onConfirm={() => CourseOrderSystemModalDelete(courseOrderSystemModalData[i].id)} okText="是" cancelText="否">
                    <a>删除</a>
                </Popconfirm>
            </div>
        )
    ...
    let modalOptsInner = {
        ...
        footer : [
            ...
            <Button key="submit" type="primary" onClick={ handleComplete } style={{ marginLeft : 10 }} loading = { courseOrderSystemAddOrEditModalLoading } disabled = { courseOrderSystemAddOrEditModalLoading }>{ courseOrderSystemAddOrEditModalType == 'add' ? '保存' : '保存' }</Button>
        ],
    ...
    return(
       <Modal {...modalOptsOut}>
            ...
            {classCeletionFailed ?
                <Modal {...modalFailure}>
                    <div>该课系已关联课程，请先将相关课程调整至其他课系</div>
                    <div className={styles.relevance}>已关联课程:
                    { failedText && failedText.length > 0 ?
                                failedText.map(function(item,index){
                                    return(
                                        <span key={index + ''}>{item.courseName} ， </span>
                                    )
                                })
                                :
                                []
