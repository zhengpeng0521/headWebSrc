...
function ScheduleRoomComponent({
	...
	let scheduleDataListByRoom = scheduleDataByRoom( scheduleDataListByDate, selectedRooms )         //按教室整理
	...
	let loopRow = time_str => {
		...
		for( let i = 0; i < 7; i++ ){
			...
			for( let j = 0; j < selectedRooms.length; j++ ){
				...
				let roomData = !!dayData && dayData[roomName];
									...
									let class_content = (
										<div className = { styles.class_content } key = { 'schedule_class_item_pop_cont_' + time_data_item_index } >
											...
											<div className = { styles.class_content_classroom } >
												{ time_data_item.classroomName || '无' }
											</div>
			...
			for( let j = 0; j < selectedRooms.length; j++ ){
				let render_col_item = (
					<div className = { styles.head_schedule_item } key = { 'head_schedule_item_' + i + j }>
						{ selectedRooms[j].roomName }
					</div>
				);
            	head_render_col.push( render_col_item );
			}
			head_render_col.push(
				<div key = { 'room_content_schedule_line_' + i } className = { !!selectedRooms && selectedRooms.length > 0 && styles.line } ></div>
			)
		}
	...
	let scroll_width = selectedRooms.length * 7 * 80;
