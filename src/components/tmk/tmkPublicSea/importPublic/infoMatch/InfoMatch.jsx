...
import styles from "./InfoMatch.less"
...
const InfoMatch = ({
  ...
  if (secondStepMatchData && secondStepMatchData.length > 0) {
    matchData = secondStepMatchData.map((item, index) => {
      ...
      if (item.key == "name" || item.key == "firstParentName" || item.key == "firstParentMobile") {
        newValue = item.value + "（必填）"
      }
      return (
        <div
          className={styles.map_item}
          key={index}
          style={index == secondStepMatchData.length - 1 ? { border: 0 } : {}}>
          <div>
            <Popover placement="top" content={item.value} trigger="hover">
              {newValue}
            </Popover>
          </div>
          <Select
  ...
  if (secondStepMisMatchData && secondStepMisMatchData.length > 0) {
    misMatchData = secondStepMisMatchData.map((item, index) => {
      if (!item.disabled) {
        return (
          <div
            className={styles.mis_map_item}
            key={index}
            style={
              index == secondStepMisMatchData.length - 1 ? { border: 0 } : {}
            }>
            <Popover placement="top" content={item[index] + ""} trigger="hover">
              {item[index] + ""}
            </Popover>
          </div>
        )
