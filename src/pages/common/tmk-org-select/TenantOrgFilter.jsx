...
const TenantOrgFilter = React.createClass({
    ...
    init() {
        ...
        //获取缓存的校区列表
        let orgPermissionList = window._init_data.tmkorgList;
        if(orgPermissionList && orgPermissionList.length > 0) {
            ...
            tmkServiceRequest(`${BASE_URL}/crm/hq/spend/deptOrgTreeQuery`, {},
                function(ret) {
                    ...
                    //更新缓存机构
                    window._init_data.tmkorgList = all_org_list;
    ...
    render () {
        let { itemDisabledFunc, getPopupContainer, hasDept, placeholder } = this.props;
        //渲染树节点
        ...
        let loopOrgSelectTreeNode = (data, parentKey) => data.map(function(item) {

            if (item.children && item.children.length > 0) {
                return (
                    <TreeNode key={item.key} title={item.label} value={item.key}  disabled={item.mark == 'city'}  >
                      {loopOrgSelectTreeNode(item.children, item.key)}
                    </TreeNode>
                  );
            } else {
                return <TreeNode key={item.key}
                                 title={item.label}
                                 value={hasDept ? item.key + '-' + parentKey : item.key}
