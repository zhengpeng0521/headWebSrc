...
let Page3DesignComponent = Form.create()(React.createClass({
	getInitialState() {
		return {
	...
	initFormData( detailData ){
		...
		let me = this;
		form.setFieldsValue({ "page4Title" : detailData.title });
		form.setFieldsValue({ "page4Title1" : detailData.title1 });
		form.setFieldsValue({ "page4Title2" : detailData.title2 });
		form.setFieldsValue({ "page4Title3" : detailData.title3 });
	...
	onChildPreview(){
		...
		let detailData = {
			type : 'Page3Component',
			seqNo : seqNo,
			title  : formData.page4Title  ,
			title1 : formData.page4Title1 ,
			title2 : formData.page4Title2 ,
			title3 : formData.page4Title3 ,
			img_intro : [
				{ imgurl : formData.shareImage1 },
				{ imgurl : formData.shareImage2 },
				{ imgurl : formData.shareImage3 },
			]
		};
	...
	imgHandleChange(info){
		...
		form.setFieldsValue({ "shareImage3" : "" });
	...
	render () {
		...
		//主标题属性
		let titleProps = getFieldProps('page4Title',{
			validate : [{
				rules : [
		...
		//标题属性
		let title1Props = getFieldProps('page4Title1',{
			validate : [{
				rules : [
		...
		let title2Props = getFieldProps('page4Title2',{
			validate : [{
				rules : [
					{ validator : this.checkTitle }
				],
		...
		let title3Props = getFieldProps('page4Title3',{
			validate : [{
				rules : [
					{ validator : this.checkTitle }
				],
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem>
						...
						<Input placeholder="第三张图片URL" type= "hidden" {...getFieldProps('shareImage3')}/>
					</FormItem>
					<FormItem {...formItemLayout} label = "标题" help = '标题, 不能超过8字' >
						<Input size = 'default' placeholder="请输入主标题" { ...titleProps } />
					</FormItem>
					<FormItem {...formItemLayout} label = "内容1" help = '不能超过30字' >
						<Input size = 'default' placeholder="请输入标题1" { ...title1Props } />
					</FormItem>
					<FormItem {...formItemLayout} label = "内容2" help = '不能超过30字' >
						<Input size = 'default' placeholder="请输入标题2" { ...title2Props } />
					</FormItem>
					<FormItem {...formItemLayout} label = "内容3" help = '不能超过30字'>
						<Input size = 'default' placeholder="请输入标题3" { ...title3Props } />
					</FormItem>
					<FormItem {...formItemLayout}
							  label="图片"
							  help="最多上传3张图, 图片大小 ≤ 1M, 支持png,jpeg,gif格式">
...
export default Page3DesignComponent;
