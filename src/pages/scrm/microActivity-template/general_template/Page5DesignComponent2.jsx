...
let Page2DesignComponent = Form.create()(React.createClass({
	...
	onChildPreview(){
		...
		let detailData = {
			...
			intro : keys.map((value,index) =>{
				return {value : formData[`configItemDetail-${index}`]}
			})
		};
	...
	importValueToKeys (){
		...
		for ( let index = 0 ; index < length ; index ++ ){
			let configItemDetail = `configItemDetail-${index}`;
			let addItem = {value : this.props.form.getFieldValue(configItemDetail) };
		...
		let formItems = keys.map((value,index) =>{
			...
			return (
				<FormItem
			          ...
			          key={index}
			        >
		            	<Input size = 'default' placeholder="详情 , 不能为空" {...configDetailProps} style={{width:'90%'}} />
		          <Icon
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
			        ...
			        <FormItem {...formItemLayout}>
			          <Button size = 'default' type="dashed" onClick={ this.addConfigItem } style={{ width: '100%' , marginLeft: '90px' }} disabled = { keys.length >= 3}>
			            <Icon type="plus" /> 新增
			          </Button>
			        </FormItem>
