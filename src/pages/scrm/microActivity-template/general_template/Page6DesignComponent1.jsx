...
let Page5DesignComponent = Form.create()(React.createClass({
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({"page5Title2" : detailData.title2 });
		form.setFieldsValue({"page5Content" : detailData.intro});
	...
	onChildPreview(){
		...
		let detailData = {
			...
			title2 : formData.page5Title2,
			img_intro : [
				{ imgurl : formData.shareImage1 },
				{ imgurl : formData.shareImage2 },
			],
			intro : formData.page5Content
		};
	...
	render () {
		...
		//标题二属性
		let titleProps2 = getFieldProps('page5Title2',{
			validate : [{
				rules : [
					{ required : true, message : '请输入标题' },
					{ validator : this.checkTitle }
		...
		//内容属性
		let contentProps = getFieldProps('page5Content',{
			initialValue : "圣诞活动详情内容",
			validate : [{
				rules : [
					{ required : true , message : '请输入内容详情', min : 1, max : 100 },
		...
		return (
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem { ...formItemLayout } label = "内容" help = '输入回车键/enter即可换行, 不超过100字' >
						<Input size = 'default' className="page5-content-textarea" type = "textarea" placeholder = "请输入内容,不超过100字" { ...contentProps }/>
					</FormItem>
					<FormItem { ...formItemLayout } label = "标题二" help = '标题, 不能超过8字' >
						<Input size = 'default' placeholder = "请输入标题" { ...titleProps2 } />
					</FormItem>
