...
let Page4DesignComponent = Form.create()(React.createClass({
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({ "page3Title1" : detailData.title1 });
		form.setFieldsValue({ "page3Title2" : detailData.title2 });
	...
	onChildPreview(){
		...
		let detailData = {
			...
			title1 : formData.page3Title1 ,
			title2 : formData.page3Title2 ,
			img_intro : [
	...
	render () {
		...
		//标题属性
		let title1Props = getFieldProps('page3Title1',{
			validate : [{
				rules : [
		...
		let title2Props = getFieldProps('page3Title2',{
			validate : [{
				rules : [
					{ validator : this.checkTitle }
				],
		...
		return (
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem {...formItemLayout} label = "内容1" help = '不能超过50字' >
						<Input size = 'default' placeholder="请输入内容1" { ...title1Props } />
					</FormItem>
					<FormItem {...formItemLayout} label = "内容2" help = '不能超过50字' >
						<Input size = 'default' placeholder="请输入内容2" { ...title2Props } />
					</FormItem>
