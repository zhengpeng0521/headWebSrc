...
let Page3DesignComponent = Form.create()(React.createClass({
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({"page3subTitle" : detailData.sub_title});
	...
	onChildPreview(){
		...
		let detailData = {
			...
			sub_title : formData.page3subTitle,
	...
	//校验副标题字数限制
	checkSub_Title( rule, value, callback ){
		if(!(/^[^\n]{1,12}$/.test(value))){
    		callback('不能超过12个字符');
    	}else if((/^[\s]{1,12}$/.test(value))){
			callback("不能为空格")
    	} else {
    		callback();
    	}
	...
	render () {
		...
		//副标题属性
		let sub_titleProps = getFieldProps('page3subTitle',{
			validate : [{
				rules : [
					{ required : true , message : '请输入副标题'},
					{ validator : this.checkSub_Title }
				],
				trigger : ['onBlur' , 'onChange']
			}]
		});
		...
		let formItems = keys.map((value,index) =>{
			...
			return (
				<FormItem
			          ...
			          key={index}
			        >
		            	<Input size = 'default' placeholder="标题" {...configTitleProps} style={{width:'28%'}} />
		            	<Input size = 'default' placeholder="详情" {...configDetailProps} style={{ marginLeft:'8px',width:'60%'}} />
		          <Icon
		            className="delete-button"
		            type="minus-circle-o"
		            disabled={ keys.length === 1 }
		            onClick = { this.deleteConfigItem.bind(this,value) }/>
		        </FormItem>
			)
		});
		return (
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem {...formItemLayout} label="副标题">
						<Input size = 'default' placeholder="请输入副标题" {...sub_titleProps}/>
					</FormItem>
					{formItems}
			        <FormItem {...formItemLayout}>
			          <Button size = 'default' type="dashed" onClick={ this.addConfigItem } style={{ width: '100%' , marginLeft: '85px' }} disabled = { keys.length >= 5}>
			            <Icon type="plus" /> 新增
			          </Button>
			        </FormItem>
