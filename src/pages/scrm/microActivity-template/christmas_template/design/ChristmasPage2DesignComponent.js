...
let ChristmasPage2DesignComponent = Form.create()(React.createClass({
	getInitialState() {
		return {
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({"page2subTitle" : detailData.sub_title});
	...
	onChildPreview(){
		...
		let detailData = {
			...
			sub_title : formData.page2subTitle,
	...
	render () {
		...
		let self = this;
		let { getFieldValue, getFieldsValue, getFieldProps, getFieldError, isFieldValidating } = this.props.form;
		...
		//副标题属性
		let sub_titleProps = getFieldProps('page2subTitle',{
			initialValue : "这个圣诞一起来做活动吧",
			validate : [{
				rules : [
...
export default ChristmasPage2DesignComponent;
