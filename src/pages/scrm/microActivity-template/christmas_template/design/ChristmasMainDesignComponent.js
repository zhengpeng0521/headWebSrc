...
import {Button,Row, Col,Input,Select,Icon,Modal ,Form ,Upload,message,Checkbox} from 'antd';

let FormItem = Form.Item;

let ChristmasMainDesignComponent = Form.create()(React.createClass({
	getInitialState() {
		return {
		...
		return (
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem>
						<Input size = 'default' type="hidden" {...getFieldProps('backgroundMusicName')} />
					</FormItem>
					<FormItem
						{...formItemLayout}
						label="校区"
					...
					<FormItem
						...
						help = '微活动名称, 不超过10字'
					>
						<Input size = 'default' placeholder = "请输入微活动的名称, 不超过10字" { ...activityNameProps }/>
					</FormItem>
					...
					<FormItem wrapperCol = {{ offset : 18 }} >
						<Button size = 'default' type = "primary" style={{ marginLeft : '7px' }} onClick = {this.onContinue}>继续<Icon style = {{ marginTop : '-3px', verticalAlign : 'middle' }} type="arrow-right" /></Button>
					</FormItem>
				</Form>
			</div>
		);
...
export default ChristmasMainDesignComponent;
