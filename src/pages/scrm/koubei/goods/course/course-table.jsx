...
let KoubeiCourseFormModal = require('./course-form');
...
let KoubeiCourseTable = React.createClass({
	getInitialState() {
		return {
	...
	queryData(query, page) {
		  ...
		  serviceRequest(BASE_URL+'/orderGoodController/courseList', {...query,pageIndex: page.pageIndex, pageSize: page.pageSize, funcKey: 'koubeimarket-course-list'},
			function(result) {
				me.setState({
					selectedRowKeys: [],
					data: result.results,
		...
		//查询各个状态下数量
		  serviceRequest(BASE_URL+'/orderGoodController/countOfStatus', {'goodsType' : '1', funcKey: 'koubeimarket-activity-list'},
			function(result) {
				me.setState({
	  ...
	  handleChange(selectedRowKeys, selectRows) {
		...
		updateStatus(record) {
			...
			serviceRequest(BASE_URL+'/orderGoodController/updateCourseStatus', {'goodsId' : record.id, 'status' : new_status, funcKey: 'koubeimarket-course-down'},
					function(result) {
		...
		batchUpdateStatus(status) {
			...
			serviceRequest(BASE_URL+'/orderGoodController/batchUpdateCourseStatus', {'goodsIds' : goodIds, 'status' : status, funcKey: 'koubeimarket-course-down'},
					function(ret) {
		...
		batchDeleteRecord() {
			...
			let new_status = 'INVALID';
			let goodIds = this.state.selectRecordIds.join(',');
			serviceRequest(BASE_URL+'/orderGoodController/batchUpdateCourseStatus', {'goodsIds' : goodIds, 'status' : new_status, funcKey: 'koubeimarket-course-down'},
					function(ret) {
						let successObj = ret.data.success || [];
						let errorObj   = ret.data.failed || [];
						let successCount = successObj.length;
	...
	render() {
	         				...
	         				let course_keshishu = record.courseHour || '';
	         				  ...
	         				  return (
	         					<div className="table-item-info" title={'名称:' + record.subject + '\n' + '类型:' + course_cat + '\n' + '年龄:' + course_age + '\n' + '课时数:' + course_keshishu}>
	         						...
	         						<p className="table-item-info-item">类型: {course_cat}</p>
	         						<p className="table-item-info-item">年龄: {course_age}</p>
	         						<p className="table-item-info-item">课时数: {course_keshishu}</p>
	         					</div>
     						  );
	         			  }
	         		},{
		...
		return (
			<div className="table-bg">
				...
				<Table columns={columns} dataSource={this.state.data}  pagination={paginationProps} onChange={this.tableOnChange} bordered scroll={{ x : 1430 }} />
				<KoubeiCourseFormModal ref="component_form_modal" visible={this.state.formVisible} changeVisible={this.handleOpenFormModal} formDataId={this.state.formDataId} afterSubmit={this.initData} />
...
export default KoubeiCourseTable;
