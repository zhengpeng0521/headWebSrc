...
let SciencePageDesignComponent = Form.create()(React.createClass({
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({ "orgContent" : detailData.orgContent });
	...
	onChildPreview(){
		...
		let detailData = {
			...
			orgContent  : formData.orgContent,
	...
	render () {
		...
		//机构简介内容
		let orgContentProps = getFieldProps('orgContent', {
			validate : [{
				rules : [
					{ required : true , min : 1 , max : 200 , message : '请输入内容, 不超过200字' },
					{ validator : this.checkContent }
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem
						...
						help = '输入回车键/enter即可换行, 不超过200字'
					>
						<Input size = 'default' type = "textarea" placeholder = '请输入内容' { ...orgContentProps } />
					</FormItem>
					<FormItem
						{ ...formItemLayout }
						label = "图片"
