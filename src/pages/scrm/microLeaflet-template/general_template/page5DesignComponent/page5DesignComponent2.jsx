...
let Page5DesignComponent = Form.create()(React.createClass({
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({ "page5SubTitle2" : detailData.sub_title2 });
		form.setFieldsValue({ "page5SubTitle3" : detailData.sub_title3 });
		form.setFieldsValue({ "page5SubTitle4" : detailData.sub_title4 });
	...
	onChildPreview(){
		...
		let detailData = {
			...
			sub_title2  : formData.page5SubTitle2,
			sub_title3  : formData.page5SubTitle3,
			sub_title4  : formData.page5SubTitle4,
			img_intro : [
	...
	render () {
		...
		//标题属性2
		let title1Props = getFieldProps('page5SubTitle2',{
			validate : [{
				rules : [
					{ required : true , message : '请输入内容', min : 1, max : 30 },
		...
		//标题属性3
		let title2Props = getFieldProps('page5SubTitle3',{
			validate : [{
				rules : [
					{ required : true , message : '请输入内容', min : 1, max : 30 },
					{ validator : this.checkTitle }
		...
		//标题属性4
		let title3Props = getFieldProps('page5SubTitle4',{
			validate : [{
				rules : [
					{ required : true , message : '请输入内容', min : 1, max : 30 },
					{ validator : this.checkTitle }
		...
		return (
			...
			<div className="templet-instance-form-content">
				<Form horizontal style={{width:'100%'}}>
					...
					<FormItem>
						<Input type= "hidden" {...getFieldProps('shareImage1')}/>
						<Input type= "hidden" {...getFieldProps('shareImage2')}/>
						<Input type= "hidden" {...getFieldProps('shareImage3')}/>
					</FormItem>
					<FormItem {...formItemLayout} label = "标题" help = '标题, 不能超过8字' >
						<Input size = 'default' placeholder="请输入标题, 不超过8字" { ...titleProps } />
					</FormItem>
					...
					<FormItem
						...
						help = '标题, 不超过30字'
					>
						<Input size = 'default' placeholder = "请输入标题" { ...title1Props } />
					</FormItem>
					<FormItem
						{ ...formItemLayout }
						label = "内容二"
