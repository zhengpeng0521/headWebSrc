...
let Page1DesignComponent = Form.create()(React.createClass({
	...
	initFormData( detailData ){
		...
		form.setFieldsValue({"page6Content" : detailData.content});
		form.setFieldsValue({"page6TitleImage" : detailData.address_url });
	...
	onChildPreview(){
		...
		let detailData = {
			...
			content : formData.page6Content,
			address_url : formData.page6TitleImage,
		};
	...
	imgHandleChange(info){
		...
		if(info.fileList.length > 0){
			if(info.file.status === "done"){
				...
				this.props.form.setFieldsValue({ 'page6TitleImage' : fileUrl })
				message.success(`${info.file.name}上传成功`);
			}else if (info.file.status === "error"){
				message.error(`${info.file.name}上传失败`);
			}
		} else if (info.fileList.length <=0){
			this.props.form.setFieldsValue({ 'page6TitleImage' : '' });
		}
		this.setState({fileList});
	},

	imgBeforeUpload(file){
		let imgUrlList = this.props.form.getFieldValue('page6TitleImage');
	...
	render () {
		...
		//副标题属性
		let subTitleProps = getFieldProps('page6Content', {
			validate : [{
				rules : [
		...
		//图片属性
		let shareImgProps = getFieldProps('page6TitleImage',{
			validate : [{
				rules : [
		...
		let defaultFileList = [];
		if(this.state.detailData && this.state.detailData.address_url){
			defaultFileList.push({
				  uid : -1,
				  name : 'xxx.png',
				  status : 'done',
				  url : this.state.detailData.address_url
	    	});
		}
		let uploadImgProps = {
			name : 'file',
