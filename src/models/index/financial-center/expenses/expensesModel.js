  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === "hq_orgser_expenses") {
  ...
  effects: {
    ...
    *getExpenseList({ payload }, { put, call, select }) {
      ...
      if (ret && ret.errorCode === 0) {
        ...
        message.error(
          ret && ret.errorMessage ? ret.errorMessage : "获取支出记录失败"
        )
      }
      ...
      if (ret && ret.errorCode === 0) {
        ...
        if(payload && payload.projectId){
          ...
          yield put({
            type: "updateState",
            payload: {
              subProjectList: subList
            }
          })
        }
      } else {
        message.error(
          ret && ret.errorMessage ? ret.errorMessage : "获取支出类别失败"
        )
      }
    ...
    *getPayWayList({ payload }, { put, call, select }) {
      yield put({ type: "updateState", payload: { listLoading: true } })
      let params = {...payload, payWayType: '1'}
      ...
      if (ret && ret.errorCode === 0) {
        ...
        message.error(
          ret && ret.errorMessage ? ret.errorMessage : "获取支出方式失败"
        )
      }
      yield put({ type: "updateState", payload: { listLoading: false } })
    },
    ...
    *addExpense({ payload }, { put, call, select }) {
      ...
      let state = select(state => state.expensesModel)
      let params = {
        ...payload,
        recordType: '1'
      }
    ...
    *getExpenseDetail({ payload }, { put, call, select }) {
      ...
      if (ret && ret.errorCode === 0) {
        ...
        message.error(
          ret && ret.errorMessage ? ret.errorMessage : "获取支出记录详情失败"
        )
      }
      yield put({ type: "updateState", payload: { btnLoading: false } })
    },
    ...
    *addProject({ payload }, { put, call, select }) {
      const { ret } = yield call(addProject, parse(payload))
      if (ret && ret.errorCode === 0) {
    ...
    *addPayWay({ payload }, { put, call, select }) {
      const { ret } = yield call(addPayWay, parse(payload))
      if (ret && ret.errorCode === 0) {
        message.success("添加成功")
        yield put({
          type: 'updateState',
    ...
    *checkModelFile({ payload }, { put, call, select }) {
      const { ret } = yield call(checkModelFile, parse(payload))
      if (ret && ret.errorCode === 0) {
        if (ret.flag) {
    ...
    *getImportField({ payload }, { put, call, select }) {
      ...
      const { ret } = yield call(getImportField, parse(payload))
      if (ret && ret.errorCode === 0) {
    ...
    *previewImport({ payload }, { put, call, select }) {
      const { ret } = yield call(previewImport, parse(payload))
      if (ret && ret.errorCode === 0) {
        let newList = []
    ...
    *expenseImport({ payload }, { put, call, select }) {
      ...
      let { ret } = yield call(expenseImport, parse(payload))
      if (ret && ret.errorCode === 0) {
    ...
    *expenseIsComplete({ payload }, { put, call, select }) {
      ...
      let expenseModel = yield select(state => state.expensesModel)
      let { ret } = yield call(expenseIsComplete, parse(payload))
      if (ret && ret.errorCode === 0) {
        let sleep = function(ms) {
          return new Promise(function(resolve, reject) {
            setTimeout(function() {
