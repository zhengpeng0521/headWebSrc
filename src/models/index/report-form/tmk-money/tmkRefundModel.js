import { queryList } from '../../../../services/report-form/tmk-money/tmkRefundService';
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            history.listen(({ pathname, query }) => {
              if(pathname === '/hq_tmk_report_refund'){
    ...
    effects: {
			...
			*getList({ payload },{ call, put, select }){
				...
				if( ret && ret.errorCode === 0 ){
					let dataSource = []
					ret.refundsInfoItemList && ret.refundsInfoItemList.forEach((item, index) => {
						item.deptRefundsItemList && item.deptRefundsItemList.forEach((child, key) => {
							dataSource.push({
								...child,
								id: index + '' + key
							})
