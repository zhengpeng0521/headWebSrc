...
export default {
  ...
  effects: {
          ...
          if(cpId != undefined && cpId != '') {
              ...
              if( ret && ret.errorCode == 9000 ){
                ...
                if(normalStuArr && normalStuArr.length > 0) {
                    let normalStuArrObj = normalStuArr.split(',');
                    normalStuArrObj && normalStuArrObj.length > 0 && normalStuArrObj.map(function(normalStuItem) {
                        classStuComList.push({stuId: normalStuItem});
                    });
                }
      ...
      *updateClassStudent({ payload }, { call, put, select }) {
          ...
          let { ret } = yield call( classStuComList, parse(params) );
            if( ret && ret.errorCode == 9000 ){
                yield put({
                    type : 'updateState',
                    payload : {
                        classStuComList: ret.results,
                    }
                });
            } else {
              message.error((ret && ret.errorMessage) || '没有获取到班级学员');
          }
      },
